<template>
    <AdminLayout>
        ...
        <FormCard :cancel-route="route('admin.recipes.index')" @submit="submit">
            ...
            <FormInput v-model="form.image" label="Imagen" type="file" accept="image/*" :preview="recipe.image ? `/storage/${recipe.image}` : null" :error="form.errors.image" />
...
const form = useForm({
    ...
    image: null,
    ingredients: props.recipe.ingredients.map(group => ({
        title: group.title || '',
        items: group.items || ['']
    })),
...
function submit() {
    form.transform(data => ({
        ...data,
        _method: 'PUT',
    })).post(route('admin.recipes.update', props.recipe.id), {
        forceFormData: true,
    });
}
</script>